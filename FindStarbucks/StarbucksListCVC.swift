...
class StarbucksListCVC: UICollectionViewController {
  ...
  //variables for locationDelegate
  ...
  let store = CoreDataStack.store
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
      //fetching instances of Starbucks from core data to populate collectionview
      ...
      navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .search, target: self, action: #selector(searchNearby(_:)))


    }

  func searchNearby(_ sender: UIBarButtonItem) {
      //Search distance within 1 mile of current location
      apiManager.currentLocation = currentLocation
      apiManager.calculateSearchFieldFromCurrentLocation(withDistance: 1)

      //search for starbucks nearby
      apiManager.getNearbyStarbucks() {
        self.store.fetchStarbucks()
        self.collectionView?.reloadData()

        //if no results display alert
