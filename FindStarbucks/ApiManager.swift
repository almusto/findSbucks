...
class ApiManager {
  ...
  //made some adjustments, but credit to swift algo club -- calulates distance between two coordinates
  ...
  func getPlaceDetails(_ placeID: String, completion: @escaping (Bool) -> ()) {
    placesClient.lookUpPlaceID(placeID, callback: { (place, error) in
      ...
      //create new starbucks in coredata
